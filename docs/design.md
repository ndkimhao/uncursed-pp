...
2. **Parameterized merge** — loop helpers whose bodies match after abstracting
   ...
   #define UNCURSED_PP_H1(r, d, e) d e;
   #define DECLARE_INTS(xs)   BOOST_PP_SEQ_FOR_EACH(UNCURSED_PP_H1, int, xs)
   #define DECLARE_FLOATS(xs) BOOST_PP_SEQ_FOR_EACH(UNCURSED_PP_H1, float, xs)
   ```

Shared helpers are named `UNCURSED_PP_H<n>` in deterministic (first-use) order so
...
```c
/* Generated by uncursed-pp from example.uncursed — do not edit. */
#pragma once
...
- Commands: `mise install && uv sync` → `uv run pytest` →
  `uv run uncursed-pp examples/example.uncursed -o /tmp/example.h && cc -E -P /tmp/check.c`.
