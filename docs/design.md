...
{{field.type}} get_{{field.name}}(const struct self *s) {
  return s->{{field.name}};
}
end
...
| tuple named/indexed access | `BOOST_PP_TUPLE_ELEM(size, idx, x)` |
...
that cursedpp never token-pastes implicitly — future `concat()` if needed).

## Generated header shape

```c
/* Generated by cursedpp from example.cursed — do not edit. */
#ifndef EXAMPLE_CURSED_H   /* from output filename */
#define EXAMPLE_CURSED_H
#include <boost/preprocessor.hpp>   /* or --pp-include value */
...macros...
#endif
```
