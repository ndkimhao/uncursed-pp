...
def runtime_header(config: EmitConfig) -> str:
    ...
    pp = config.pp_prefix
    include = config.pp_include or f"{config.pp_include_dir}/tuple/replace.hpp"
    return (
        ...
        f"#include <{include}>\n"
        "\n"
        f"#define {hp}KW_PUT(state, ...) {hp}KW_PUT_I(state, __VA_ARGS__)\n"
        f"#define {hp}KW_PUT_I(state, i, v) {pp}TUPLE_REPLACE(state, i, v)\n"
    )
